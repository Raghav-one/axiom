...
const all=[...authored.map(deepenChapter).map(expandFoundation).map(deepenComputing).map(deepenMathematics).map(deepenDataAndML).map(deepenAIMap),...frontier].sort((a,b)=>domains.findIndex(d=>d.id===a.group)-domains.findIndex(d=>d.id===b.group));
